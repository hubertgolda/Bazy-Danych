...
ALTER TABLE ksiegowosc.pracownicy
ADD COLUMN nowy_nr_telefonu VARCHAR(20);

UPDATE ksiegowosc.pracownicy
SET nowy_nr_telefonu = CONCAT('(+48) ', telefon);

select p.imie, p.nazwisko, p.telefon, p.nowy_nr_telefonu
from ksiegowosc.pracownicy p;

-- b.:

UPDATE ksiegowosc.pracownicy
SET telefon2 = CONCAT_WS('-', 
SUBSTRING(telefon2, 1, 3), 
SUBSTRING(telefon2, 4, 3), 
SUBSTRING(telefon2, 7, 3));
...
select p.imie, p.nazwisko, md5(pn.kwota::varchar)
from ksiegowosc.pracownicy p
inner join ksiegowosc.wynagrodzenie wy on p.id_pracownika = wy.id_pracownika 
inner join ksiegowosc.pensja pn on pn.id_pensji = wy.id_pensji;

-- f.:

select p.imie, p.nazwisko, pn.kwota, pr.kwota
from ksiegowosc.pracownicy p
left join ksiegowosc.wynagrodzenie wy on p.id_pracownika = wy.id_pracownika
left join ksiegowosc.pensja pn on pn.id_pensji = wy.id_pensji
left join ksiegowosc.premia pr on pr.id_premii = wy.id_premii;

-- g.:

select concat('Pracownik ',p.imie,' ',p.nazwisko,', w dniu ',
wy."data",' otrzymał pensję całkowitą na kwotę ',
pn.kwota+pr.kwota,', gdzie wynagrodzenie zasadnicze wynosiło: ',
pn.kwota,' zł, premia: ',pr.kwota,' zł, nadgodziny: ',g.liczba_godzin) 
from ksiegowosc.pracownicy p
inner join ksiegowosc.godziny g on p.id_pracownika = g.id_pracownika
inner join ksiegowosc.wynagrodzenie wy on g.id_godziny  = wy.id_godziny
inner join ksiegowosc.pensja pn on wy.id_pensji = pn.id_pensji
inner join ksiegowosc.premia pr on pr.id_premii = wy.id_premii;
